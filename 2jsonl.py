...
"""
Modified by me
Original Authors: https://github.com/DocNow/twarc/blob/master/utils/json2csv.py
...
def main():
		...
		if args.output:
				...
				csv_file = sys.stdout
		sheet = csv.writer(csv_file, delimiter=";")
...
def text(t):
	if 'retweeted_status' in t and t['retweeted_status']:
		...
		real_text = (t.get('full_text') or t.get('extended_tweet', {}).get('full_text') or t['text']).replace('\n', ' ')

	real_text2 = ''.join(c for c in unicodedata.normalize('NFC', real_text) if c <= '\uFFFF')
	real_text5=''.join([i if ord(i) < 128 else ' ' for i in unidecode(str(real_text2))])
...
if __name__ == "__main__":
		#data = pd.read_json('C:/Users/jm/Music/xag.jsonl', lines=True)
		#writer = pd.ExcelWriter('output.xlsx')
		#data.to_excel("C:/Users/jm/Music/xag.xlsx")
		main()
